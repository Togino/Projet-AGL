    ...
    if ($actionId > 0) {
        try {
            ...
            $pdo->prepare("
                ...
                VALUES (?, ?, 'approve')
            ")->execute([$actionId, $currentMat]);

            $countStmt = $pdo->prepare("
                SELECT COUNT(*) FROM action_confirmations
                WHERE action_queue_id = ? AND decision = 'approve'
            ");
            ...
            if ($approvalCount >= (int) $queue["required_confirmations"]) {
                ...
                $pdo->prepare("UPDATE action_queue SET status = ?, processed_at = NOW() WHERE id = ?")
                    ->execute([$exec["status"] === "success" ? "executed" : "failed", $queue["id"]]);
    ...
    <section class="dashboard">
        <div class="dashboard-head">
            <div>
                ...
                <p>Suivi des actions sensibles en attente de confirmations.</p>
            </div>
        </div>
